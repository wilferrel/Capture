...
-(void)initialSetup{
    _cellWidth=[[UIScreen mainScreen] bounds].size.width/2;
}
...
-(void)setupCollectionView {
    ...
    [_refreshControl addTarget:self action:@selector(refreshDropboxList) forControlEvents:UIControlEventValueChanged];
    [self.filesCollectionView addSubview:_refreshControl];
...
-(void)reloadTableWithArray:(NSArray*)arrayOfItems{
    if ([_filesCollectionArray count]>0) {
        [_filesCollectionArray removeAllObjects];
    }
    _filesCollectionArray=[NSMutableArray arrayWithArray:arrayOfItems];
    [_filesCollectionView reloadData];
}
...
-(void)takePictureWithCamera{
    ...
    UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"DOH!" message:@"Camera is not available on iPhone Simulator" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
    [alert show];
#else
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    ...
    return [_filesCollectionArray count];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row<[_filesCollectionArray count]) {
        DropBoxImage *tempImage=[_filesCollectionArray objectAtIndex:indexPath.row];
